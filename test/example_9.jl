...
c5 = Process(sim, "c5")
c6 = Process(sim, "c6")
activate(c1, 0.0, get_served, 100.0, 1, server, in_clients, out_clients)
activate(c2, 0.0, get_served, 100.0, 2, server, in_clients, out_clients)
activate(c3, 0.0, get_served, 100.0, 3, server, in_clients, out_clients)
activate(c4, 0.0, get_served, 100.0, 4, server, in_clients, out_clients)
activate(c5, 0.0, get_served, 100.0, 5, server, in_clients, out_clients)
activate(c6, 0.0, get_served, 100.0, 6, server, in_clients, out_clients)
run(sim, 500.0)
