...
function buy(customer::Process, budget::Float64)
	println("$(now(customer)) $customer: Here I am at the shops, I have $budget to spend")
	t = 5.0
	for i = 1:4
		if budget < 10.0
			break
	...
	println("$(now(customer)) $customer: All I have left is $budget, I am going home")
end
...
function execute(source::Process, finish::Float64)
	...
	while now(source) < finish
		...
		activate(c, now(source), buy, 100.0*rand())
