...
function generate(source::Process, number::Int64, time_between_arrivals::Float64)
	for i = 1:number
		c = Process(simulation(source), @sprintf("Customer%02d", i))
		activate(c, now(source), visit, 12.0)
		hold(source, time_between_arrivals)
	end
end
...
max_number = 5
max_time = 400.0
time_between_arrivals = 10.0

# Model/Experiment

sim = Simulation(uint(16))
s = Process(sim, "Source")
activate(s, 0.0, generate, max_number, time_between_arrivals)
run(sim, max_time)
